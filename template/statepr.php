...
<h2>
    Состояние заказов 
        ...
        <?php 
            ...
            $sta = getStatus($st);
        ?>   
            <option value="<?=SITE_URL?>/?action=statepr&status=<?=$sta['id']?>"><?=$sta['name']?></option>
            <option disabled>- - - - - - - - - -</option>
        <?php 
            $statusy = getStatusy();
            foreach ($statusy as $status):
        ?>
            <option value="<?=SITE_URL?>/?action=statepr&status=<?=$status['id']?>"><?=$status['name']?></option>
        <?php endforeach; ?>
        </select>
    по номеру ТТН
</h2>
...
?>
	...
	<img src="<?=SITE_URL?>/image/mobile_phone_arrow.ico" onclick="SendSMS('<?=$phone?>','<?=trim($zakaz['ttn'])?>',event);" class="send-sms-icon"><?=$zakaz['phone']?>
            ...
            <td style="font-size: 11px; color:#666;">
                <div style="width:150px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" title="<?=$zakaz['delivery_adress']?>">
...
$(document).ready(function(){ 
    //setTimeout(function(){
            ...
            if(ttn.length > 0){
                    $.ajax({
                        url: "<?=SITE_URL?>/modules/pr/status.php",
                        method: 'POST',
                        data : {ttn:ttn},
                        beforeSend: function(){
                        ...
                        success: function(data){
                            ...
                            $('#text-ttn-<?=trim($zakaz['ttn'])?>').html(data);    
