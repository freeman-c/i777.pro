...
//sleep(2);
session_start();
function AjaxSearchProductOrders($source){
    db_connect();
    $query = "SELECT order_id FROM zakazy WHERE utm_source LIKE '%$source%' ORDER BY date DESC"; 
...
$zakazy = AjaxSearchProductOrders($_GET['source']);
if($zakazy){
foreach ($zakazy as $z):
...
endforeach;
     ...
     echo '<tr><td colspan="19" align="center"><h3>По метке "'.$_GET['source'].'" не найдено данных!</h3></td></tr>';}
?>