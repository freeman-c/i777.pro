...
//sleep(2);
session_start();
function AjaxSearchOrdersOnAdvertise($source, $product_id){
    db_connect();
    $query = "SELECT distinct z.order_id FROM zakazy as z, product_order as po WHERE";
    $ifs = false;
    if ($source != 'all'){
        $query .= " z.utm_source LIKE '%$source%' AND"; 
    }
    if ($product_id != 'all') {
        $query .= " po.product_id = $product_id AND";
    }
    $query .= " z.order_id = po.order_id ORDER BY z.date DESC";
...
function AjaxSearchOrders($order_id){
    ...
    $sort = "AND cart < 1";
    $between = "";
    if($_GET['complete']){ $sort = "AND cart < 1 AND date_complete > 0";}
...
$zakazy = AjaxSearchOrdersOnAdvertise($_GET['source'], $_GET['product_id']);
...
endforeach;
     ...
     echo '<tr><td colspan="19" align="center"><h3>По источнику "'.$_GET['source'].'" не найдено данных!</h3></td></tr>';}
?>