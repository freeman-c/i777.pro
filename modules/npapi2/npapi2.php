...
class NovaPoshtaApi2 {
	...
	protected function checkInternetDocumentParams(array & $params) {
		...
		( ! $params['VolumeGeneral']) AND $params['VolumeGeneral'] = '0.001';
		( ! $params['OptionsSeat']) AND $params['OptionsSeat'] = array('volumetricVolume' => '0.001', 'volumetricWidth' => '10',
	...
	function newInternetDocument($sender, $recipient, $params) {
		...
		if ( ! $sender['Sender']) {
			...
			print_r($senderCounterpartyExisting);
		...
		print_r($paramsInternetDocument);
	...
	protected function printGetLink($method, $documentRefs, $type) {
		$data = 'https://my.novaposhta.ua/orders/printDocument/orders[]/'.implode(',', $documentRefs)
