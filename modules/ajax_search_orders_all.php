...
function AjaxSearchOrders(){
    ...
    $sort = "cart < 1";
    $between = "";
    if($_GET['complete']){ $sort = "cart < 1 AND date_complete > 0";}
...
if(isset($_SESSION['user']['manager'])){$session_manager = "AND user='".$_SESSION['user']['manager']."'";}  
    ...
    $query = "SELECT * FROM `zakazy` 
                    WHERE $sort $session_office $session_payment $session_delivery $session_manager $between
                AND `bayer_name` LIKE '%$search%'
                OR (`site` LIKE '%$search%')    
		OR (`id` LIKE '%$search%')
