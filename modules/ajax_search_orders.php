...
function AjaxSearchOrders(){
    ...
    if($_GET['complete']){ $sort = "AND status='".$_GET['status']."' AND date_complete > 0";}
...
if(isset($_SESSION['user']['manager'])){$session_manager = "AND user='".$_SESSION['user']['manager']."'";}
    ...
    $search = $_GET['search'];
    $query = "SELECT * FROM `zakazy` WHERE cart < 1 $sort $session_office $session_payment $session_delivery $session_manager $between AND `bayer_name` LIKE '%$search%'
                OR (`site` LIKE '%$search%') 
		OR (`id` LIKE '%$search%')		
                ...
                OR (`user` LIKE '%$search%')
                OR (`delivery_adress` LIKE '%$search%')
                OR (`ttn` LIKE '%$search%')
                OR (`utm_source` LIKE '%$search%')
		OR (`utm_medium` LIKE '%$search%')
		OR (`utm_term` LIKE '%$search%')
		OR (`utm_content` LIKE '%$search%')
...
if($_GET['status'] && $order['status']==$_GET['status']){
?>
...
endforeach; 
     ...
     echo '<tr><td colspan="19" align="center"><h3>По запросу "'.$_GET['search'].'" не найдено данных!</h3></td></tr>';}
?>